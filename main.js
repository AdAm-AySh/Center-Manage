...
let title = JSON.parse(localStorage.getItem("title_Website")) || "سنتر Adam";
let currentTeacherIndex = null;
let currentGroupIndex = null;
const notesTextarea = document.getElementsByTagName("textarea")[0];
const titleElement = document.getElementById("title");
const titleInput = document.getElementById("systemNameInput");
...
function saveToLocalStorage() {
  ...
  localStorage.setItem("title_Website", JSON.stringify(titleInput.value.trim()));
}

titleElement.textContent = `إدارة ${title}`;
...
if (updateTitleBtn) {
  updateTitleBtn.addEventListener("click", () => {
    if (titleInput.disabled) {
      ...
      titleInput.focus();
      updateTitleBtn.textContent = "حفظ";
    } else {
      const newName = titleInput.value.trim();
      if(!newName) return alert("اكتب اسم السنتر الأول");
      ...
      updateTitleBtn.textContent = "تعديل";

      titleInput.style.backgroundColor = "#222";
    }
  })
}
      ...
      group.students.forEach((student, sIndex) => {
        ...
        if (matchSearch) {
          ...
          tr.innerHTML = `
              ...
              <td>
              <button class="btn-danger btn-global-delete" onclick="deleteGlobalStudent(${tIndex}, ${gIndex}, ${sIndex})">حذف</button>
              </td>
            `;
